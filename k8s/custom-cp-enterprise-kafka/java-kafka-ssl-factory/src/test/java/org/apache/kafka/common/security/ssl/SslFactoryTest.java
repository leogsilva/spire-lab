...
public abstract class SslFactoryTest {
    ...
    public static final String AGENT_SOCK = "unix:/run/spire/sockets/agent.sock";
    ...
    protected SslEngineFactory createServerFactory() {
        String address = AGENT_SOCK;
        SpireSslEngineFactory serverFactory = new SpireSslEngineFactory();
        Map<String,String> serverConfig = new HashMap<String,String>();
        serverConfig.put(SpireSslEngineFactory.AGENT_SOCK, AGENT_SOCK);
        serverConfig.put(SpireSslEngineFactory.SPIFFE_IDS, "spiffe://example.org/ns/sslfactory/sa/producer");
    ...
    public void testSslFactoryConfiguration() throws Exception {
        SslEngineFactory factory = createServerFactory();
        //host and port are hints
        SSLEngine engine = factory.createServerSslEngine("localhost", 0);
        assertNotNull(engine);
        assertTrue(Utils.mkSet(engine.getEnabledProtocols()).contains(tlsProtocol));
    ...
    public void testSslFactoryWithoutProviderClassConfiguration() {
        ...
        serverConfig.put(SpireSslEngineFactory.AGENT_SOCK, AGENT_SOCK);
        serverConfig.put(SpireSslEngineFactory.SPIFFE_IDS, "spiffe://example.org/ns/sslfactory/sa/producer");
        serverConfig.put(SslConfigs.SSL_PROTOCOL_CONFIG, "someprot");
